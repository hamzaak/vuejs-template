  ...
  <el-card class="box-card">
    <page-header>
      ...
      <template v-slot:right>
        <el-tooltip effect="dark" content="Create Transaction" placement="bottom">
          <el-button type="primary" size="mini" plain @click="_onRedirect(`/transactions/create/${filters.accountId}`)">
            <v-icon name="plus" />
          </el-button>
        </el-tooltip>
        <el-tooltip effect="dark" content="Money Transfer" placement="bottom">
          <el-button type="primary" size="mini" plain @click="_onRedirect(`/transactions/transfer/${filters.accountId}`)">
            <v-icon name="exchange-alt" />
          </el-button>
        </el-tooltip>
        <el-tooltip effect="dark" :content="filters.open == 0 ? 'Open Filters' : 'Close Filters'" placement="bottom">
    ...
    <el-collapse v-model="filters.open" @click="_toggleFilters">
      <el-collapse-item title="Filters" :name="1">
        <template slot="title">
          <span :style="filters.open == 0 ? 'color:#606266;':'color:#409eff;'">Filters</span>
        </template>
          ...
          <el-row :gutter="24">
            <el-col :span="12">
              <el-form-item label="Date">
                <el-date-picker
                  v-model="filters.date" type="daterange" range-separator="To" start-placeholder="Start date" end-placeholder="End date"
            ...
            <el-col :span="12">
              ...
              <el-form-item label="Description">
                <el-input v-model="filters.description" placeholder="Search in description" />
              </el-form-item>
            </el-col>
          </el-row>
          <div style="text-align:right;">
            <el-button @click="_onResetFilters" size="mini" round>Reset</el-button>
            <el-button type="primary" @click="_onApplyFilters" size="mini" round>Apply</el-button>
          </div>
        </el-form>
      </el-collapse-item>
    </el-collapse>
    <el-table v-loading="table.loading" :data="table.data" :row-class-name="_rowClass">
      <el-table-column prop="date" label="Date" :formatter="_dateFormatter"  width="120" />
      <el-table-column label="Amount" width="120" :formatter="_currencyFormatter"/>
...
export default {
  ...
  methods: {
    ...
    _onItemDelete(id) {
      this.$confirm("This will permanently delete the transaction. Continue?", "Confirm", {
